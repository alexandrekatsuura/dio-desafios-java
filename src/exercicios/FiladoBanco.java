...
public class FiladoBanco {
    /*
        ...
        será dada não pela ordem de chegada, mas sim pelo número recebido via sms.
    ...
    public static void Executar() {
        ...
        for(int i =0; i < entradas; i++){
            int pessoas = Integer.valueOf(input.nextLine());
            List numeros = Arrays.stream(Arrays.asList(input.nextLine().split(" ")).stream().mapToInt(Integer::parseInt).toArray()).boxed().collect(Collectors.toList());
            List numerosOrdenado = new ArrayList();
            numerosOrdenado.addAll(numeros);
            numerosOrdenado.sort(Comparator.reverseOrder());

            int cont=0;
            for(int k =0; k < numeros.size(); k++){
                if(numeros.get(k).equals(numerosOrdenado.get(k))){
                    cont++;
                }
            }
        ...
        saida.stream().forEach(System.out::println);
