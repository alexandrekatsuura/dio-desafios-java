...
/*
    ...
    uniformes a serem feitas para aquela turma.
    ...
    seguido por um espaço e pelo tamanho do uniforme "P" "M" ou "G". A entrada termina quando o valor de N for
...
public class UniformesDeFinalDeAno {
    public static void Executar() {
        ...
        for(int i =0; i < entradas+1; i++){
            ...
            if(!nome.equals("0")) {
                String camisa = input.nextLine();
                todas.add(camisa + " " + nome);
                    }else{
                        i = entradas+2;
                    }
        ...
        todas.stream().filter(pessoa -> pessoa.toString().contains("branco P")).forEach(System.out::println);
        todas.stream().filter(pessoa -> pessoa.toString().contains("branco M")).forEach(System.out::println);
        todas.stream().filter(pessoa -> pessoa.toString().contains("branco G")).forEach(System.out::println);
        todas.stream().filter(pessoa -> pessoa.toString().contains("vermelho P")).forEach(System.out::println);
        todas.stream().filter(pessoa -> pessoa.toString().contains("vermelho M")).forEach(System.out::println);
        todas.stream().filter(pessoa -> pessoa.toString().contains("vermelho G")).forEach(System.out::println);
